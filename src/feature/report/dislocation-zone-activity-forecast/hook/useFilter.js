  ...
  switch (action.type) {
    ...
    case 'DATE_TO':
      if (action.value) {
        ...
        prevState[action.key] = `${format(new Date(year, month, day, 23, 59, 59), 'Y-MM-dd\'T\'HH:mm:ss.SSSXXX')}`;
      } else {
        delete prevState[action.key];
      }
...
export const useFilter = (tableName, tableColumns, filtersData, initialState, customFilter) => {
  ...
  const handleExport = () => exportTableToExcel(exportData, new Date());
  ...
  const onFilterSave = () => {
    ...
    if (!formState.title || !formState.shortcut) {
      alert(t`provide_title_and_shortcut`);
      return;
    }
      ...
      <div className={showFilter ? 'visible w-full' : 'hidden'}>
        <div className='w-full flex flex-row columns-3 border-t-2 border-gray-200'>
                    ...
                    <div key={id} className={'flex flex-row'}>
                      <div id={id} onClick={onFilterApply} onMouseEnter={onFilterFocus} onMouseLeave={onFilterBlur} className={`${defaultFilterId === id ? 'border-blue-700 border-r-4' : ''} flex w-full justify-between hover:opacity-50 cursor-pointer px-6 py-2`}>
                        ...
                        <Nullable on={editMode}>
                          <Button.Xs id={id} onClick={onModeEdit} className={'bg-slate-400 px-4 py-0'}>{t`edit_filter`}</Button.Xs>
                        </Nullable>
            ...
            {(mode === Mode.Create || mode === Mode.Edit) &&
              <div className='flex flex-col p-6'>
                ...
                <div className='flex space-x-2 mt-5'>
                  <Nullable on={mode === Mode.Edit}><Button.NoBg onClick={deleteFilter} className={'shadow-none text-gray-400 font-normal py-0'}>{t`delete`}</Button.NoBg></Nullable>
                  <div className='flex-grow'></div>

                  <Button.NoBg onClick={onModeCancel} className={'shadow-none text-gray-400 font-normal px-2 py-0'}>{t`cancel`}</Button.NoBg>
                  <Button.Xs onClick={onFilterSave} className={'bg-oxford text-white px-4 py-0 hover:bg-gray-700'}>{t`save`}</Button.Xs>
                </div>
              </div>
            }
          </div>
          ...
          <div className='flex-grow flex flex-col border-l-2 border-slate-200 py-6 px-6'>
              ...
              {filtersData.map(({key, label, filter, Component, Child, values, displayValue}, index) => {
                ...
                if (filter === 'select' || filter === 'multiselect') {
                  ...
                  return <SelectBox
                    ...
                    value={currentValue} 
                    placeholder={filter === 'multiselect' ? t`select_multiple_choices` : t`any_select`}
                  >
                    {map(
                      value => (
                ...
                if (filter === 'autocomplete') {
                  ...
                  return <ComboBox 
                    key={key}
                    labelText={label}
                    placeholder={t`search_multiple_choices`}
                    className={'w-full'}
                    value={currentValue}
                    onChange={v => dispatch({type: filter.toUpperCase(), value: v, key: key})}
                    multiple={true}
                ...
                // date from
                  ...
                  return <DatePicker 
                    ...
                    key={key} 
                    label={label} 
                    defaultValue={state[key] ? new Date(state[key]) : null}
                    placeholder={t`select_date`} 
                    onChange={v => dispatch({type: filter.toUpperCase(), value: v, key: key})} 
                  />
                }
            ...
            {/* filtering columns */}
            <div >
              <span className='text-sm'>{t`column_order`}</span>
              <Filter onValues={setColumns}>{pickedColumns}</Filter>
            </div>
            <div className='mt-4'>
              <Nullable on={defaultFilterId}>
                <Button.Xs id={defaultFilterId} className={'bg-oxford px-6 py-0 hover:bg-gray-700'} onClick={onSaveFilterShortcut}>{t`save_filter`}</Button.Xs>
              </Nullable>
  ...
  const queryParams = useMemo(() => {
    ...
    for (const [key, value] of Object.entries(state)) {
      ...
      if (filter.custom !== undefined && isFunction(filter.custom)) {
        const customParams = filter.custom(value, state);
