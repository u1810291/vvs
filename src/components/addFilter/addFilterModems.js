...
export const AddFilterListModems = (props) => {
  ...
  const addFilterFunc = () => {
    ...
    filterListModems.map((value, index, array) => {
      if (filterListModems.length - 1 === index) {
        const id = value.id;
        setSelectedFilterModems(id);
      }
    });
  };

  const checkFilters = useCallback(async() => {
    if (filterListModems.length === 1) {
      setSelectedFilterModems(null)
    } else {
    const topID = filterListModems[0].id;
    setSelectedFilterModems(topID);
    }
  },[filterListModems, setSelectedFilterModems])
        ...
        {filterListModems.map((filter, index) => {
          return (
            <div className="w-full" key={filter.id}>
              {filterEditingModems !== filter.id ? (
                <button
                  onClick={() => setSelectedFilterModems(filter.id)}
                  className={filterEditingModems ? "hidden" : "w-full"}
                  key={filter.id}
                >
                  <div
                    className={
                      selectedFilterModems === filter.id ? "shadow" : "w-full"
                    }
                  >
                    ...
                    <div className="flex flex-col sm:flex-row w-full border-l border-b justify-between items-center">
                      ...
                      <a className="flex p-1 rounded-sm text-xs sm:mx-2 px-2 mb-2 font-normal items-center text-gray-400 hover:text-gray-500 bg-gray-200">
                        <p onClick={() => setFilterEditingModems(filter.id)}>
                          redaguoti filtrą
                        </p>
                      </a>
                    </div>
                ...
                <div className="ml-6 w-full">
                  ...
                  <div className=" flex flex-col sm:flex-row justify-around items-center w-20 sm:w-full mt-8">
                    ...
                    <button
                      onClick={() => setFilterEditingModems(null)}
                      className="text-gray-400 text-sm hover:text-gray-500"
                    >
                      Atšaukti
                    </button>
